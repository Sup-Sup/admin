...
export const mutations = {
	...
	async updateUserInfo(data = false) {
		console.log('---  更新用户信息，走到这一步了嘛  ---',  data );
		if (data) {
			...
			try {
				...
				console.log(' -- 获取用户信息后 -- ', realNameRes);
				this.setUserInfo({
					...res.result.data[0],
	...
	loginSuccess(e = {}){
		...
		console.log('-- common store.js 往这里推数据 --', e);
		this.updateUserInfo()

		uni.$emit('uni-id-pages-login-success')

		if (config.setPasswordAfterLogin && !passwordConfirmed) {
			console.log('--- config 中没有这个参数哇 setPasswordAfterLogin  ---');
			return uni.redirectTo({
		...
		if (autoBack) {
			...
			console.log('--- 执行这里 autoBack -- ', autoBack );
			console.log('--- 执行这里 uniIdRedirectUrl -- ', uniIdRedirectUrl );
